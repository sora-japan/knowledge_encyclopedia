...
### ② docstring の整備（バックエンド完了後・Step 6 に入る前）
...
- [ ] **削除によるカウントリセットの抜け道を塞ぐ**（下記「⚠️ 既知の穴」参照）
...
# 14. ⚠️ 既知の穴：削除で日次カウントがリセットされる

## 問題

日次上限は `discoveries` テーブルの**残っている行**を `created_at` で数えている。
物理削除を採用しているため、**削除するとカウントも消える。**

```
30件登録 → 全部削除 → カウント0 → さらに30件登録できる
```

LLM と埋め込みで**既に2回API呼び出し済み**なのに、その事実が記録から消える。
削除を繰り返せば実質無制限に呼べる。

**localhost で自分だけが使う間は実害ゼロ。公開デプロイの瞬間に本当の穴になる。**

## 対策：論理削除（ソフトデリート）

`deleted_at` カラム（`TIMESTAMPTZ`, nullable）を追加し、
DELETE で行を消さず日時を入れる。カウントは**全行**を対象にするので上限が戻らない。

### 影響範囲（書き漏らすと削除済みが表示される）

| 箇所 | 必要な変更 |
|---|---|
| GET一覧 | `WHERE deleted_at IS NULL` を追加 |
| GET詳細 | 同上。削除済みは404扱い |
| PUT | 同上。削除済みは編集不可 |
| DELETE | `db.delete()` → `deleted_at` に日時を代入 |
| 日次カウント | **条件を追加しない**（ここが目的） |
| ベクトル検索 | `WHERE deleted_at IS NULL` を追加 |

**日次カウントだけ条件を付けない**のが要点。他は全部付ける。
この非対称性はコードを読んでも意図が分からないので、コメントを残すこと。

## ゴミ箱機能とは別物

| | 目的 | 必要なもの |
|---|---|---|
| **論理削除** | API課金の抜け道を塞ぐ | カラム1本 + 各クエリに条件 |
| ゴミ箱UI | 誤削除からの復元 | 一覧画面・復元API・完全削除API |

**論理削除だけ先にやる。** ゴミ箱UIは後回しでよい。
`deleted_at` さえ持っていれば、後からUIを足せる。

## 着手時期

**RAG完成後、公開デプロイ前。**

RAG実装中にDBスキーマとAPI 5本を同時に触ると、
どちらの変更で壊れたか切り分けられなくなる（Step 3 で「LLM抜きで器を作る」としたのと同じ理由）。
