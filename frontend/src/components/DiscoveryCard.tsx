...
export default function DiscoveryCard({ discovery }: Props) {
  ...
  return (
    // カード全体がリンク。grid のセルいっぱいに広げるため block h-full を付ける
    <Link
      href={`/discoveries/${discovery.id}`}
      className="group block h-full rounded-2xl focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-zinc-950"
    >
      <article className="flex h-full flex-col gap-3 rounded-2xl border border-black/[.06] bg-white p-5 shadow-sm transition group-hover:shadow-md dark:border-white/10 dark:bg-white/[.04]">
        <div>
          <span
